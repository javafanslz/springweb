...
function YQZQ() {
    ...
    // 初始化页面的方法
    YQZQ.prototype.initPage = function(pageType) {
        try {
    ...
    YQZQ.prototype.initDisableComps = function() {
        $.each(_yqzq.elements, function(key, val) {
            if(val && val.initDisable){
                if(val.type =='radio'){
                    var _radios = $("input[name="+key+"]");
                    $.each(_radios,function(key,value){
                        _yqzq.disableComp($(this), true);
                    });
                }else{
                    var elem = $("#" + val.id);
                    if(elem.length == 1){
                        _yqzq.disableComp(elem, true);
                    }
                }
            }
        });
    };
    YQZQ.prototype.disableComp = function(elem, val) {
    ...
    //验证联通客户必选上传延期邮件
    //延期错误
    YQZQ.prototype.validate_YQEmail = function() {
